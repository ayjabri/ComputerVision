...
nodes=[('S','A',6),('S','B',2),('S','C',5),('B','E',3),('A','D',9),
    ...
    ('F','D',4),('G','E',5)]
simple = nx.DiGraph()
simple.add_weighted_edges_from(nodes)
simple.pos = {'S':([0., 0.]),
            'A': ([1.,0.]),
            'B': ([0.,-1.]),
            'C': ([0., 1.]),
...
simple.weights = nx.get_edge_attributes(simple,'weight')
plt.figure(figsize=(8,8))
nx.draw_networkx_edge_labels(simple,simple.pos,edge_labels=simple.weights,font_size=18)
nx.draw(simple,pos=simple.pos,with_labels=True,node_size=5000,font_size=24,width=3,
...
class SearchAgent(object):
    ...
    def AST(self,G,heurestics,start):
        q = {(start,):0}
        v = []
        while q:
            q = dict(sorted(q.items(),key=lambda x:x[1],reverse=True))
            ...
            for n in neighbours:
                if n not in (v + [i[-1] for i in q]):
                    f = neighbours[n]['weight'] + heurestics[n]
                    new_path = path + (n,)
                    q[new_path]=f
        return 'Failure'
...
def main():
    ...
    algo = algos[int(input('''Select an algorithm to run:
            1- BFS
            2- DFS
            3- A* - works only with Romania's map
            '''))-1]
    if algo =='ast':
        start = str(input('Romania: enter starting city: '))
        model = ro
        PlotPath(model,*(ai.AST(model,ro_heuristics,start)))
    else:
        model = models[int(input('''Select a graph:
            1- US cities
            2- Romania
            3- Simple
            '''))-1]
        start = str(input('Starting point : '))
        end = str(input('End point: '))
    
    if algo == "bfs":
        
        PlotPath(model,*(ai.BFS(model,start,end)))

    elif algo == "dfs":

        PlotPath(model,*(ai.DFS(model,start,end)))
